...
export class CostCalculator {
  ...
  private calculateModelUsageComponents(modelUsage: ModelTokenUsage, pricingModelName: string): CostComponents {
    ...
    let displayRates = hasKnownPerCallContext
      ? this.selectPricingRates(pricing, rawContextTokens(this.asUsageCall(modelUsage)))
      : this.baseRates(pricing)
    let previousRates: PricingRates | undefined

    for (const call of calls) {
      const contextTokens = rawContextTokens(call)
      const rates = hasKnownPerCallContext ? this.selectPricingRates(pricing, contextTokens) : this.baseRates(pricing)
      uncachedInput += (contextTokens / 1_000_000) * rates.input
      ...
      usesTieredPricing ||= hasKnownPerCallContext && this.usesNonBasePricing(pricing, contextTokens)
